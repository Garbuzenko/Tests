...
def commands_logic(user_command):
    if user_command == 'p':
        owner_name = get_doc_owner_name()
        return 'Владелец документа - {}'.format(owner_name)
    elif user_command == 'ap':
        uniq_users = get_all_doc_owners_names()
        return'Список владельцев документов - {}'.format(uniq_users)
    elif user_command == 'l':
        show_all_docs_info()
    elif user_command == 's':
        directory_number = get_doc_shelf()
        return 'Документ находится на полке номер {}'.format(directory_number)
    elif user_command == 'a':
        ...
        return print(secretary_program_start.__doc__)
    elif user_command == 'q':
        return 'break'
    else:
        return 'break'
...
def secretary_program_start():
    ...
    while res != 'break':
        ...
        res = commands_logic(user_command)
        print(res)
